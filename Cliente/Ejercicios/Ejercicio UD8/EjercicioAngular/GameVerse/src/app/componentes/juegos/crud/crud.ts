...
export class CRUD {
  ...
  private juegosService = inject(ServerJuego); // Inyectamos el servicio UsuarioServices para poder utilizar sus métodos y obtener la lista de usuarios, así como realizar operaciones de edición y eliminación. Esto nos permitirá interactuar con la API para gestionar los usuarios desde este componente.
  ...
  displayedColumns: string[] = ['id', 'titulo', 'genero', 'plataforma', 'año', 'precio', 'rating', 'desarrollador', 'descripcion', 'estado', 'imagen']; // Define las columnas que se mostrarán en la tabla
  dataSource: MatTableDataSource<Juego>; // Fuente de datos para la tabla, se inicializa en el constructor
  ...
  constructor() {
    this.dataSource = new MatTableDataSource<Juego>([]); // Inicializa la fuente de datos con un array vacío, se llenará con los usuarios obtenidos del servicio  
  }
  ...
  eliminarUsuario(juego: Juego): void {
    const snackRef = this.snackBar.open(
      `¿Eliminar a ${juego.titulo}?`,
      'Confirmar',
      {
    ...
    snackRef.onAction().subscribe(() => { // Si el usuario hace clic en "Confirmar", se ejecuta esta función para eliminar el usuario 
      this.juegosService.deleteJuego(juego.id!).subscribe({ //
        next: () => {
          this.cargarJuegos();
        },
        error: () => {
