/** 
 * IMPORTANTE MODIFICAR PARA MODULOS 
 * REVISAR QUE ESTE TODO BIEN EN SU SITIO
*/
...
export const getAlumnos = async (req, res) => {
  try {
    const alumnos = await Alumno.find().sort({ apellidosNombre: 1 }); //orden ascendente -1 descendente
...
export const getAlumnoCurso = async (req, res) => {
  try {
    ...
    const alumnos = await Alumno.find({ idCurso: idCurso }).sort({
      apellidosNombre: 1,
    });
...
export const addAlumno = async (req, res) => {
  try {
    ...
    if (!cursoExiste) {
      return res
        .status(400)
        .json({ message: `El curso "${idCurso}" no existe` });
    }
    ...
    res
      .status(500)
      .json({ message: "Error al insertar el alumno", error: error.message });
  }
};
...
export const updateAlumno = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error al actualizar el alumno", error: error.message });
  }
};
...
export const delAlumno = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Error al borrar el alumno", error: error.message });
  }
};
