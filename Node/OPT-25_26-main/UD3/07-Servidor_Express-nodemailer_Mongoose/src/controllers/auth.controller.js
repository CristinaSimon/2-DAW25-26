...
export const register = async (req, res) => {
  try {
    ...
    const html = contenidoHTML(name, activacionToken);
    await enviarEmailActivacion(email, "Activar Cuenta", html);
...
export const activaCuenta = async (req, res) => {
  try {
    ...
    if (!usuario) {
      return res.status(400).json({ message: "Token de activacion invalido" });
    }
    //comprobar que el usuario tiene la cuenta inactiva
    if (usuario.active) {
      return res.status(400).json({ message: "La cuenta ya esta activa" });
    }
    if (usuario.activacionToken < Date.now()) {
      return res.status(400).json({ message: "El token ya expiro" });
    }
    ((usuario.active = true),
    ...
    return res.status(200).json({
      message:
        "La cuenta a sido activada exitosamente. Ya puedes iniciar sesion",
    });
  } catch (error) {
